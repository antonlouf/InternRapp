...
import { BehaviorSubject, Observable, Subject,combineLatest,merge,startWith, switchMap, catchError, of, debounceTime } from "rxjs";
...
export abstract class BaseList<T> {
    ...
    public get filtered$(): Observable<string> 
    {
         return this.filtersSub$.asObservable();
    }
    public pagingUpdated(e: PageEvent): void 
    { 
        this.paginationsSub$.next({pageNumber:e.pageIndex+1,pageSize:e.pageSize})
    }
    public filterUpdated(newFilters: string): void 
    { 
        this.filtersSub$.next(newFilters); 
    }
    protected abstract getGridItems$(filter: PaginationFilterRequest, dependable?: unknown ): Observable<ResourceItemPagingResponse<T>>;
    ...
    protected configureListData$(refreshObservables?: Array<Observable<unknown>>): Observable<ResourceItemPagingResponse<T>> 
    {
        const refresh$ = refreshObservables && refreshObservables.length > 0? merge(...refreshObservables): of(undefined);
         ...
         { return this.getGridItems$( { pageSize:currentPage.pageSize,pageIndex:currentPage.pageNumber,filterString:filters }, dependable )
